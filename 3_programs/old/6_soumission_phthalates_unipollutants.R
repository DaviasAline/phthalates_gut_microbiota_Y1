...
pollutants_vec <- bdd_alpha %>% 
  select(all_of(phthalates_vec)) %>% 
  select(!contains(c("MEOHP", "MECPP", "MEHHP", "MEHP", "MMCHP", "ohMiNP", "oxoMiNP", "cxMiNP", "ohMINCH", "oxoMINCH"))) %>% colnames()
alpha_vec <- bdd_alpha %>% 
...
## on passe de 31 expositions à 17 expositions après prise en compte de leur corrélation
...
results_multi <- results_multi %>%
  mutate(
    FWER.p.value = p.value * 17 * 2,
    FWER.p.value = ifelse(FWER.p.value > 1, 1, FWER.p.value)) 

results_multi <- results_multi %>%
  mutate(
    FWER.p.value_shape = case_when(p.value< 0.0015 ~ "p.value <0.0015",
                              p.value > 0.0015 ~ "p.value >0.0015"), 
    FWER.p.value_shape = fct_relevel(FWER.p.value_shape,
                                "p.value >0.0015", 
                                "p.value <0.0015"))


#### Correction pour comparaison multiple (toxonomie pour les polluants associés à l'alpha div) ----
test_expo <- bdd_alpha %>% 
  filter(!is.na(ch_feces_Shannon_5000_ASV_Y1)) %>%
...
results_multi <- results_multi %>%
  mutate(
    FWER.p.value = p.value * 4 * 2,
    FWER.p.value = ifelse(FWER.p.value > 1, 1, FWER.p.value)) 

results_multi <- results_multi %>%
  mutate(
    FWER.p.value_shape = case_when(p.value< 0.006 ~ "p.value <0.006",
...
forestplot <- function(results_list, outcome_name) {
  results <- results_list %>%
    ...
    geom_hline(yintercept = 0, linetype="dashed") +
    geom_pointrange(position = position_dodge(width = 0.7), size = 0.4,
                    aes(color = ifelse(FWER.p.value_shape == "p.value >0.0015", "p.value >0.0015", "p.value <0.0015"))) +
    labs(x = "Exposures", y = outcome_name) +
    theme_bw() +
    coord_flip()  +
...
ggsave("4_output/phthalates/forestplot_alpha_phthalates.tiff", 
...
forestplot_alpha_exploratory <- 
  (forestplot_alpha_exploratory_1 + forestplot_alpha_exploratory_2 + forestplot_alpha_exploratory_3) / leg_exploratory + 
  plot_layout(heights = c(9, 1))

ggsave("4_output/phthalates/forestplot_alpha_exploratory_phthalates.tiff", 
